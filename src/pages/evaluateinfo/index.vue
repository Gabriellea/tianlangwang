  ...
  <div class="counter-warp">
    <div v-for='(list,index) in goods_list' :key='index'>
      <div class='top'>
        ...
        <div class='right'>
          <p>{{list.goods_name}}</p>
          <van-rate :value="list.index" @change="onChange" />
        </div>
      </div>
      <div class='cont'>
        <van-cell-group>
  ...
  methods: {
      ...
      wx.chooseImage({  
        ...
        success: function (res) {  
          var tempFilePaths = res.tempFilePaths
            ...
            wx.uploadFile({
              url: 'http://jiazhuang.ccpingtai.cn/api/v1/upload', //仅为示例，非真实的接口地址
              ...
              success: function(res){
                ...
                // that.imgurllist[index].img.push({'url':that.imgxinxi.data.attach_uri});
                that.goods_list[index].img.push({'url':that.imgxinxi.data.attach_uri});
                that.goods_list[index].id.push({'id':that.imgxinxi.data.id});
                // that.imgurllist[index].push({'url':that.imgxinxi.data.attach_uri,'id':that.imgxinxi.data.id});
                setTimeout(function () {
                  wx.hideLoading()
                }, 500)
              }
    ...
    tijiaopinglun(){
      ...
      for(var n = 0;n<this.goods_list.length;n++){
        this.message+=this.goods_list[n].message+',';
        this.value+=this.goods_list[n].index+',';
        for(var s = 0;s<this.goods_list[n].id.length;s++){
          ids[n] += ','+this.goods_list[n].id[s].id;
        }
        this.imgids+=ids[n].slice(10)+'|';
      }
      this.$wxAjax('evaluates', {
        ...
        goods_id:this.$mp.query.goods_id,
        level:''
      }).then((data) => {
        if (data.status == 0) {
          let _this = this
