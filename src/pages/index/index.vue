<template>
  ...
  <!--轮播图地址 和搜索-->
    ...
    <!--秒杀专区-->
    <div class='xsmiaosha'>
      <img class='mstitle' src='/static/images/miaosha.jpg'>
      <div class='mslist'>
        <div class='item' v-for='(item,index) in list' :key="index"  @click="$go('shopdetails',{id:index})">
          <p class='time' v-html="times2[index]"></p>
          <img :src='item.img'>
          <p><span class='qingfou'>抢购价</span>￥{{item.price}}</p>
          <p style='color:#999999;line-height:25px'>￥{{item.oldprice}}</p>
        </div>
      </div>
    </div>
    <!--好评专区、积分专区、优惠券专区-->
    <div class='hpzq'>
      <div  @click="$go('shoplist',{id:item.url,name:item.title})" v-for='(item,index0) in goodNavs' :key="index0"><img :src='item.image_uri'></div>
    </div>
    <!--最美季节-->
    <div class='zm'>
    ...
    <div class='shop'>
      <div class='shopbox' v-for='(categorie,key) in categories' :key="key">
        <img class='shopbanner' @click="$go('shoplist',{id:categorie.id})" :src='categorie.ad_uri'>
        <div class='shoplist'>
...
export default {
  data () {
    return {
      ...
      uploadImgUrl: [],
      navlists:[],
      times1:[1561539673,1561539673,1561539673,1561539673],
      list:[
        {'img':'http://jiazhuang.ccpingtai.cn/static/uploads/manage/image/1/20181102/86438537b92bdd2037382a85b0f8da63.jpg','price':'505.00','oldprice':'80.00'},
        {'img':'http://jiazhuang.ccpingtai.cn/static/uploads/manage/image/1/20181102/86438537b92bdd2037382a85b0f8da63.jpg','price':'45.00','oldprice':'60.00'},
        {'img':'http://jiazhuang.ccpingtai.cn/static/uploads/manage/image/1/20181102/86438537b92bdd2037382a85b0f8da63.jpg','price':'66.00','oldprice':'9922.00'},
        {'img':'http://jiazhuang.ccpingtai.cn/static/uploads/manage/image/1/20181102/86438537b92bdd2037382a85b0f8da63.jpg','price':'38.00','oldprice':'562.00'}
      ],
      goodNavs:'',
      oneNavs:'',//不规则广告位
      oneNav0img:'',//不规则广告位
  ...
  onShow() {
      ...
      this.getIndexInfo();
      this.getlocation();
      console.log(this.$getStorage('parent_id'))
  },
  methods: {
    bindViewTap () {
      const url = '../logs/main'
    ...
    getLocal: function (latitude, longitude) {
      ...
      this.qqmapsdk.getCityList({
        success: function(res) {//成功后的回调
          res.result[1].forEach(function (value) {
            if(value.fullname==_this.city){
    ...
    getIndexInfo(type_id) {
      if (!this.$getStorage('token')) {
        ...
        wx.login({
          async success(res) {
            wx.request({
              url: 'http://jiazhuang.ccpingtai.cn/api/v1/code2Session?code='+res.code,
              method:"GET",
              header: {
                'Content-Type': 'json'
              },
              success: function (ret) {
                  _this.$wxAjax('login', {
                    ...
                    parent_id:_this.$getStorage('parent_id')
                    }).then((data) => {
                      console.log(data)
                      if (data.status == 0) {
