<template>
  ...
  <div class='top'>
    <!--轮播图地址 和搜索-->
      ...
      <div class='shoplist' v-if='shopinfo'>
          <img :src='shopinfo.image_uri'>
          <div class='text'>
            <p class='shoplist_title'>{{shopinfo.goods_name}}</p>
            <p class='shoplist_short'>{{shopinfo.sku_x}}</p>
            <p class='shoplist_price'><span class='price'>￥{{shopinfo.sale_price}}</span>x{{shopinfo.num}}</p>
          </div>
      </div>
      <van-cell-group>
        <van-field
     ...
     -->
      <van-cell title="优惠券：" :border='false' :value="coupon" is-link/>
      
      <div style='height:10px;background:#f8f8f8'></div>
      <van-cell title="商品总额" :border='false' :value="allprice"/>
        ...
        if(this.$mp.query.type==1){
          this.$wxAjax('detail', {
            ...
            if (data.status == 0) {
                 ...
                 this.allprice = this.shopinfo.sale_price*this.shopinfo.num;
                 this.realprice = this.allprice;
                 this.goods[0] = {"id":"0","goods_id":this.$mp.query.id,"goods_sku_id":this.$mp.query.sku,"goods_nums":this.$mp.query.num}
          ...
          this.$wxAjax('buycarList', {
            ...
            if (data.status == 0) {
              ...
              for(var i =0;i<this.shoplist.length;i++){
               ...
               this.goods[i] = {"id":this.$mp.query.cartId,"goods_id":this.shoplist[i].goods_id,"goods_sku_id":this.shoplist[i].goods_sku_id,"goods_nums":this.shoplist[i].goods_nums}
...
.tianjiadizhi{line-height:40px;padding-left:10px}
