<template>
    ...
    <van-tabs :active="active">
      ...
      <van-tab title="评价">
        <div class='evaluate' v-for='(list,key) in evaluates_list' :key="key">
          ...
          <p class='evaluate_txt'>{{list.content}}({{list.level_txt}})</p>
  ...
  <div class='footer'></div>
    <van-goods-action>
      ...
      <van-goods-action-button text="加入购物车" @click="addShopCart()"  type="warning" />
      <van-goods-action-button text="立即购买" @click="allligm()"/>
    </van-goods-action>
    <van-popup :show="show" @close="onClose" position='bottom'>
      <div class='sku_shop'>
        ...
        <div>
          ...
          <p style='color:#aeaeae'><span>销量<van-icon style='margin:2px 5px 0px;float:left' name="orders-o" />{{shop.total_sale_nums}}</span></p>
        </div>
      ...
      <div class='sku_category'>
        ...
        <p class='num'>
          <span>购买数量</span>
          <van-stepper v-model="num" @minus='add()' @plus='plus()' />
        </p>
        <div class='sku_foot'>
          <p class='addcart' @click="addShopCart()" >加入购物车</p>
          <p class='ljgm' @click='allligm'>立即购买</p>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    allligm(){
      if(this.fenleiid||this.shop.goods_spec_format_array.length==0){
        this.show = false
        this.$go('placeorder',{type:1,id:this.shop.id,sku:this.fenleiid,num:this.num})
